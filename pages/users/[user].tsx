...
const User: NextPage = () => {
    ...
    const { data, isLoading } = useQuery(['userCollections', user],
    ...
    return (
        <Layout heading="Explore" content="Explore wrapprs. Wrap anything." back={true}>
            <Box display="flex" flexDirection={"column"} alignItems="center" justifyContent="center" gap={"md"}>
                ...
                {data ? <SimpleGrid columns={[1, 2, 3, 6]} spacing={10} px={[4, 6]} py={2}>{
                    data.map((wrappr: any) => <CollectionCard address={wrappr?.wrappr?.id} id={wrappr.collectionId} tokenURI={wrappr?.uri === '' ? wrappr?.wrappr?.baseURI : wrappr?.uri} chainId={chainId.toString()} />)}
                </SimpleGrid> : (isLoading ? <Text>Fetching...</Text> : <Text>We didn't find any Wrappr owned by you on {getChainName(chainId)}</Text>)}
