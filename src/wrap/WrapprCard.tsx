...
export default function WrapprCard({ wrappr }) {
  ...
  return (
    <Flex direction="column" border="1px solid" borderColor={"gray.300"} padding={0} borderBottomRadius="lg" justifyContent={"space-between"} >
      ...
      <Text fontWeight="500" noOfLines={2}>{isLoading ? 'Loading...' : data['description']}</Text>
      </Flex>
      <Button variant="outline" onClick={() => router.push(`/wrappr/4/${wrappr['id']}`)} borderTopRadius={0} background="gray.300" width={'100%'}>
        Expand
      </Button>
    </Flex>
  )
